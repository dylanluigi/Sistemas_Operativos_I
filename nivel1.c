 ...
 * mediante check_inernal, también s tiene en cuenta el caso en el que
 * hacemos un salto de línea directamente.
 *
 *
 * line : puntero que apunta a la línea que pasamos por consola(stdin)
 *
 *
 * retorna: valor dado por check_internal (si es un comando interno 1(-1 si hubo errores) si no lo es 2)
 */
int execute_line(char *line)
{
    char *args[ARGS_SIZE];
...
int internal_cd(char **args)
{
    printf("internal_cd()->cambia de directorio actual de trabajo\n");
    return 1;
}
...
int internal_export(char **args)
{
    printf("internal_export()->Asigna variables de entorno\n");
    return 1;
}
